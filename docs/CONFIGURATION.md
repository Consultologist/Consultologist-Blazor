...
| `Public__ApiHost` | The canonical public host of this deployment (`east.ca.api.consultologist.ai`) — the data-residency statement every job record carries as `apiHost` (#514), attested on `Public/Engine`. Configuration, not the request's authority: both hostnames serve one app, and the record must say one name per region. The server cannot derive it | — | prod (unset = every record says *not recorded*, never a fallback to the Azure name) |
...
- **The mailbox also sends delivery-address confirmation codes (#486)**:
  ...
  unless the account chose not to be emailed (`delivery.emailPdf`, #518;
...
| `Operators__AppUserIds` | Comma/space-separated AppUserIds allowed to call operator surfaces (`GET /api/Operator/PinHealth`, #384; `GET /api/Operator/Usage` and the /operators panel, #553; `POST /api/Operator/Accounts/{id}/Close` — the one mutating surface, #559, Disabled accounts only). Not a role: an ordinary account, listed here. Matched exactly | nobody | no |
...
```azurecli
az storage entity insert --account-name consultjobrecscaeast --auth-mode login \
...
```azurecli
az storage entity query --account-name consultjobrecscaeast --auth-mode login \
...
```azurecli
az storage entity delete --account-name consultjobrecscaeast --auth-mode login \
