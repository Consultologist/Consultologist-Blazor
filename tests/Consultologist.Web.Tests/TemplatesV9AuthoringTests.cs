...
public class TemplatesV9AuthoringTests : ClientRenderTestContext
{
    ...
    public async Task AV9TypeOnAV8Package_IsRefusedAtTheDesk()
    {
        ...
        await WorkflowService.DidNotReceiveWithAnyArgs().PublishPackageAsync(default!);
        Assert.Contains(
            "Input 'prior_notes' declares a type that requires specVersion 9. Use \"Upgrade to specVersion 13\" and publish.",
            Refusals(page));
    }
    ...
    public async Task AV9ConditionOnAV8Package_IsRefusedAtTheDesk()
    {
        ...
        await WorkflowService.DidNotReceiveWithAnyArgs().PublishPackageAsync(default!);
        Assert.Contains(
            "Document 'consult_note' declares a condition form that requires specVersion 9. Use \"Upgrade to specVersion 13\" and publish.",
            Refusals(page));
    }
