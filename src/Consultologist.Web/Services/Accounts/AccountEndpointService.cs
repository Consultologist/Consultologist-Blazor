...
public sealed class AccountEndpointService : IAccountEndpointService
{
    ...
    public async Task DisconnectCernerLinkAsync()
    {
        using var request = new HttpRequestMessage(HttpMethod.Delete, GetAccountBaseUrl() + "/Cerner");
        await AddAuthorizationAsync(request);
        ...
        {
            var error = await response.Content.ReadAsStringAsync();
            _logger.LogError("Cerner disconnect failed with status {StatusCode}", response.StatusCode);
            throw new HttpRequestException(ExtractError(error) ?? $"Cerner disconnect failed: {response.StatusCode}");
        }
    }
